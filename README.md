...
기존 비공개 저장소의 0.3.2 이하 APK와 공개 0.3.3은 서명 인증서가 다릅니다. 기존 앱을 제거한 뒤 공개 0.3.3을 한 번 설치해야 합니다. 공개 0.3.3 이후 버전은 동일한 공개판 전용 서명 인증서를 계속 사용하므로 덮어쓰기 업데이트가 가능합니다.
...
1. 기존 비공개판 Health Bridge가 설치돼 있으면 제거합니다.
2. 공개 저장소 Release의 `HealthBridge-v0.3.3.apk`를 설치합니다.
3. 앱에서 Google 스프레드시트 ID를 입력하고 저장합니다.
4. Health Connect 읽기 권한을 승인합니다.
5. `Google Sheets 연결`을 눌러 Google 계정을 승인합니다.
6. 동기화 조건을 저장하고 필요하면 전체 다시 대조를 실행합니다.

Google OAuth는 APK의 패키지명과 서명 SHA-1을 확인합니다. 공개판 전용 SHA-1은 `PUBLIC_OAUTH_SETUP.md`에 기록돼 있습니다.
...
정식 Release가 성공하면 APK의 패키지명, 버전, 서명 인증서, SHA-256 체크섬을 다시 검증하고 `.github/release-selftest-report.json`에 결과를 기록합니다.
